...
myMatrix myMatrix::operator +(const myMatrix& mat) const
{
	...
	myMatrix result(rows, cols, pData);
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			result.pData[i][j] += mat.pData[i][j];
		}
	}
	return result;
}
myMatrix myMatrix::operator -(const myMatrix& mat) const
{
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			result.pData[i][j] = pData[i][j] - mat.pData[i][j];
		}
	}
	return result;
}
myMatrix myMatrix::operator *(const myMatrix& mat) const
{
	...
	for (int i = 0; i<rows; i++)
	{
		for (int j = 0; j<mat.cols; j++)
		{
			...
			{
				sum += pData[i][t] * mat.pData[t][j];
			}
			result.pData[i][j] = sum;
		}
	}
...
{
	myMatrix result(mat.rows, mat.cols);
	for (int i = 0; i < mat.rows; i++) {
		for (int j = 0; j < mat.cols; j++) {
			result.pData[i][j] = value * mat.pData[i][j];
		}
	}
	return result;
}
